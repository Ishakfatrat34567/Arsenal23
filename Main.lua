...
RunService.RenderStepped:Connect(function()
	...
	if mouse.Target then
		local candidateModel = mouse.Target:FindFirstAncestorOfClass("Model")
		if candidateModel then
			hoveredPlayer = Players:GetPlayerFromCharacter(candidateModel)
		end
	end
	...
	for target, object in pairs(espObjects) do
		...
		local _, headOnScreen = currentCamera:WorldToViewportPoint(head.Position)
		local visibleTarget = canSeeTarget(character, head)
		if combatStates.AimAssist.Get() and hoveredPlayer == target and headOnScreen and visibleTarget then
			bestTargetHead = head
		end
...
RunService:BindToRenderStep("StrikewareAimAssist", Enum.RenderPriority.Camera.Value + 1, function()
	...
	if not targetHumanoid or targetHumanoid.Health <= 0 or not canSeeTarget(targetCharacter, aimAssistTarget) then
		return
	end
	local currentCamera = Workspace.CurrentCamera
	if not currentCamera then
	...
	local headScreenPos, onScreen = currentCamera:WorldToViewportPoint(aimAssistTarget.Position)
	if not onScreen then
		return
	end
	local mousePos = UserInputService:GetMouseLocation()
	local delta = Vector2.new(headScreenPos.X, headScreenPos.Y) - Vector2.new(mousePos.X, mousePos.Y)
	local smooth = 0.06
	...
	if typeof(mouse_event) == "function" then
		mouse_event(moveX, moveY)
		return
	end
	local lookAt = CFrame.lookAt(currentCamera.CFrame.Position, aimAssistTarget.Position)
	currentCamera.CFrame = currentCamera.CFrame:Lerp(lookAt, 0.02)
end)
