...
local visualStates = {
	BoxESP = createToggle(visualsScroll, "Box ESP", false),
	Tracers = createToggle(visualsScroll, "Tracers", false),
...
RunService.RenderStepped:Connect(function()
	for target, object in pairs(espObjects) do
		...
		local topPos, onScreenTop = camera:WorldToViewportPoint(rootPart.Position + Vector3.new(0, 3.2, 0))
		local bottomPos, onScreenBottom = camera:WorldToViewportPoint(rootPart.Position - Vector3.new(0, 3.2, 0))
		...
		local isBox = visualStates.BoxESP.Get()
		object.Box.Visible = isBox
		if isBox then
			object.Box.Position = UDim2.fromOffset(x, y)
			object.Box.Size = UDim2.fromOffset(boxWidth, boxHeight)
		end
		...
		if isDistance then
			local studs = (camera.CFrame.Position - rootPart.Position).Magnitude
		...
		local isTracer = visualStates.Tracers.Get()
		object.Tracer.Visible = isTracer
		if isTracer then
			local bottomCenter = Vector2.new(camera.ViewportSize.X / 2, camera.ViewportSize.Y - 30)
			local targetPoint = Vector2.new(topPos.X, topPos.Y + boxHeight / 2)
			local delta = targetPoint - bottomCenter
			object.Tracer.Position = UDim2.fromOffset(bottomCenter.X, bottomCenter.Y)
			object.Tracer.Size = UDim2.fromOffset(delta.Magnitude, 2)
			object.Tracer.Rotation = math.deg(math.atan2(delta.Y, delta.X))
		end

		local highlight = ensureHighlight(target, character)
		if highlight then
			highlight.Enabled = visualStates.Chams.Get()
		end
	end
end)
