...
RunService.RenderStepped:Connect(function()
	...
	for target, object in pairs(espObjects) do
		...
		if combatStates.AimAssist.Get() and headOnScreen then
			local cursorDistance = (Vector2.new(headPos.X, headPos.Y) - Vector2.new(mousePos.X, mousePos.Y)).Magnitude
			if cursorDistance < 90 and cursorDistance < bestDistance then
	...
	if combatStates.AimAssist.Get() and bestTargetHead then
		local lookAt = CFrame.lookAt(camera.CFrame.Position, bestTargetHead.Position)
		camera.CFrame = camera.CFrame:Lerp(lookAt, 0.025)
	end
end)
